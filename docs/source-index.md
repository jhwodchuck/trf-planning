# Source Index

Last reviewed: **2026-07-27**

Use sources in this order:

1. Current TRF forms, rules, approvals, and written site-specific guidance.
2. Manufacturer manuals, labels, and specifications.
3. Government safety, terrain, soil, flood, and environmental data.
4. Reputable secondary specification databases.
5. Scale-derived estimates, clearly marked as estimates.

## Texas Renaissance Festival

| Source | Authority | Use |
|---|---|---|
| [Fields of New Market campground map](https://www.texrenfest.com/p/camping-and-more/camping/campground-map) | Official TRF | General orientation, roads, regions, and landmarks |
| [Camping overview](https://www.texrenfest.com/p/camping-and-more/camping) | Official TRF | Current camping program overview and contacts |
| [2026 campground rules and FAQs](https://www.texrenfest.com/p/camping-and-more/campground-rules--faqs) | Official TRF | Operating and safety rules |
| [Guilds and land-request forms](https://www.texrenfest.com/p/camping-and-more/guilds) | Official TRF | Current land-request process and downloads |
| [2026 General Campground Rules PDF](https://cdn.saffire.com/files.ashx?cb=db741534&f=2026_General_Campground_Rules.pdf&rid=TexasRenaissanceFval&t=fg) | Official TRF | Detailed campground rules |
| [2026 Season Pass Standards PDF](https://cdn.saffire.com/files.ashx?cb=0cf1e817&f=2026_Season_Pass_Standards.pdf&rid=TexasRenaissanceFval&t=fg) | Official TRF | Standards applying to season-pass campsites |
| [2026 Individual Land Request Form PDF](https://cdn.saffire.com/files.ashx?cb=acb34c98&f=2026_Individual_Land_Request_Form.pdf&rid=TexasRenaissanceFval&t=fg) | Official TRF | Submission requirements and individual-plot area limits |
| [2026 Communal Plot Request Form PDF](https://cdn.saffire.com/files.ashx?cb=e160fc01&f=2026_Communal_Plot_Request_Form.pdf&rid=TexasRenaissanceFval&t=fg) | Official TRF | Alternative communal/guild process |
| [Participant handbook page](https://www.texrenfest.com/p/join-us/for-our-vendors) | Official TRF | Participant handbook and operational references |
| [2025 Participant Handbook PDF](https://cdn.saffire.com/files.ashx?cb=33687d15&f=2025_Participant_Handbook.pdf&rid=TexasRenaissanceFval&t=fg) | Official TRF, prior year | Supporting requirements until a 2026 handbook is published |

### TRF contact points

- Camping questions and written rule confirmation: `camping@texrenfest.com`
- Campground inquiries: `936-218-0972`
- Campground emergency/security/fire/EMS: `936-727-1800`

Store written interpretations in the repository. The current sources appear to conflict on whether an early individual request may use 600 sq ft or is capped at 525 sq ft.

## Camper

| Source | Authority | Use |
|---|---|---|
| [Owner-supplied Forest River data-plate transcription](../equipment/evidence/2012-wildcat-data-plate.md) | Primary manufacturer-label evidence | Verifies `WCP29MK`, GVWR, axle ratings, tire size, and rim size; VIN and unit identifier withheld from the public repository |
| [2012 Wildcat Sterling 29MK floor plan image](https://spec.dlrwebservice.com/3/rv/fp/2012-Forest-River-Wildcat-Sterling-29MK-fp.png) | Dealer-data floor-plan service | Slide locations, door side, and orientation reference |
| [RVUSA 2012 Wildcat Sterling 29MK specifications](https://www.rvusa.com/rv-guide/2012-forest-river-wildcat-sterling-29mk-fifth-wheel-specs-fp31523) | Secondary specification database | Published body dimensions, weights, tanks, and equipment |
| [J.D. Power 2012 Wildcat Sterling 29MK specifications](https://www.jdpower.com/rvs/2012/wildcat-by-forest-river/m-29mk/6019298/specs) | Secondary specification database | Cross-check model and specifications |
| [Optimum RV 2012 Wildcat Sterling 29MK listing](https://www.optimumrv.com/product/used-2012-forest-river-rv-wildcat-sterling-edition-29mk-2426569-5) | Dealer listing | Additional cross-check and photographs |

**Primary identification obtained:** the manufacturer data plate confirms model code `WCP29MK`, resolving `29MK` versus `293MK` in favor of `29MK`. Still needed are actual body/deployed measurements, the original owner's manual if available, and title or sales paperwork only as supporting evidence for the consumer-facing Sterling Edition wording.

## Tow vehicle

| Source | Authority | Use |
|---|---|---|
| [2018 Ford Super Duty brochure transcription](https://manuals.plus/m/5000affcbdc76c55c196c1938eb3183ebca9700b1bf774e929800295fb8ff4f6) | Brochure mirror | Cab/box-specific dimensions |
| [Edmunds 2018 F-250 Crew Cab specifications](https://www.edmunds.com/ford/f-250-super-duty/2018/crew-cab/features-specs/) | Secondary database | Cross-check crew-cab long-bed dimensions |

**Primary evidence still needed:** cab configuration, actual bumper-to-bumper length, mirror width, hitch configuration, and parking requirement.

## Generator and electrical equipment

| Source | Authority | Use |
|---|---|---|
| [Cummins Onan P4500i product page](https://www.cummins.com/en-ame/generators/products/onan-p4500i-inverter-portable-generator) | Manufacturer | Product overview |
| [Cummins P4500i specification sheet PDF](https://mart.cummins.com/imagelibrary/data/assetfiles/0059500.pdf) | Manufacturer | Dimensions, output, weight, fuel, and parallel capability |
| [Cummins P4500i owner's manual PDF](https://www.cummins.com/sites/default/files/2025-04/p4500i-owners-manual.pdf) | Manufacturer | Operating clearances, exhaust, fuel, grounding, and maintenance |
| [CDC carbon-monoxide guidance](https://www.cdc.gov/carbon-monoxide/about/index.html) | U.S. government | Generator distance from structures/openings and CO precautions |
| [CPSC generator safety guidance](https://www.cpsc.gov/Newsroom/News-Releases/2026/CPSC-Warns-of-Generator-Carbon-Monoxide-and-Fire-Hazards-Ahead-of-Hurricane-Season) | U.S. government | Portable-generator fire and CO hazards |

## Terrain, soil, flood, and environmental screening

The repository's `maps/` workspace already records and automates several of these layers.

| Source | Authority | Use |
|---|---|---|
| [USGS National Map Viewer](https://www.usgs.gov/tools/national-map-viewer) | U.S. government | Elevation, contours, imagery, and downloadable layers |
| [USGS 3DEP products](https://www.usgs.gov/3d-elevation-program/about-3dep-products-services) | U.S. government | Elevation-model provenance and resolution |
| [USDA NRCS Web Soil Survey](https://www.nrcs.usda.gov/resources/data-and-reports/web-soil-survey) | U.S. government | Soil type, drainage, ponding, and engineering limitations |
| [FEMA Flood Map Service Center](https://msc.fema.gov/portal/home) | U.S. government | Effective flood-hazard screening |
| [USFWS Wetlands Mapper](https://www.fws.gov/program/national-wetlands-inventory/wetlands-mapper) | U.S. government | Wetland and low-area screening |

Regional data does not replace field measurements of road edges, ditches, roots, ruts, local ponding, or the exact approved boundary.

## Evidence-handling rule

For every final-plan measurement, record:

- value and unit;
- object and endpoints;
- source URL, document, photograph, or field record;
- date obtained;
- confidence: `verified-field`, `verified-document`, `published`, `scale-derived`, or `assumed`;
- verifier, when applicable.
